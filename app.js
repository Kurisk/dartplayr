...
// This is browser-local auth for a static app. A hosted backend would be needed
// for secure, cross-device accounts.
...
function getAccounts() {
  try {
    var data = localStorage.getItem(APP_STORAGE_PREFIX + "_accounts");
...
function saveAccounts(accounts) {
  localStorage.setItem(APP_STORAGE_PREFIX + "_accounts", JSON.stringify(accounts));
}
...
function makeAccountId(name) {
  return "u_" + name.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "") + "_" + Date.now();
}
...
function handleAuthSubmit() {
  ...
  var existing = accounts.find(function(a) { return a.name.toLowerCase() === name.toLowerCase(); });
  ...
  if (pin.length < 4) {
    setAuthMessage("Use a PIN with at least 4 characters.");
    return;
  }

  if (authMode === "signup") {
    if (existing) {
      setAuthMessage("That account already exists. Log in instead.");
      return;
    }
    ...
    activateAccount(existing);
    return;
  }

  if (!existing || existing.pinHash !== encodePin(pin)) {
    setAuthMessage("No matching account found for that name and PIN.");
    return;
  }
...
function restoreActiveAccount() {
  var activeId = localStorage.getItem(APP_STORAGE_PREFIX + "_active_user");
  if (!activeId) {
    showScreen("auth-screen");
    return;
  }
  ...
  var accounts = getAccounts();
  var account = accounts.find(function(a) { return a.id === activeId; });
  if (!account) {
    localStorage.removeItem(APP_STORAGE_PREFIX + "_active_user");
    showScreen("auth-screen");
    return;
